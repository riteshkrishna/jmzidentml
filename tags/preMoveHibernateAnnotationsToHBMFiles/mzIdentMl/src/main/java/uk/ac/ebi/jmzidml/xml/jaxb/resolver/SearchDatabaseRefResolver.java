 ...
 * @since 1.0
 */
public class SearchDatabaseRefResolver extends AbstractReferenceResolver<SearchDatabase> {
    ...
    @Override
    public void updateObject(SearchDatabase object) {
        // if we automatically resolve the references, then update the object with the referenced object
        if (MzIdentMLElement.SearchDatabase.isAutoRefResolving()) {
     ...
     */
    @Override
    public void checkRefID(SearchDatabase object) {
    ...
    public void afterUnmarshal(Object target, Object parent) {
        if (SearchDatabase.class.isInstance(target)) {
            updateObject((SearchDatabase)target);
